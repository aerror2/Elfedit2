...
static bfd_boolean  do_hacking = FALSE;
...
static struct groupx *   section_groups;
static struct groupx **  section_headers_groups;
...
static Elf_Internal_Shdr *
find_section_by_index (Filedata * filedata, unsigned int index)
{
    unsigned int i;

     if (filedata->section_headers == NULL)
       return NULL;
    if(index < filedata->file_header.e_shnum)
    {
   
         Elf_Internal_Shdr *sec = filedata->section_headers + index;
        return sec;
    }
...
parse_args (Filedata * filedata, int argc, char ** argv)
{
  ...
  while ((c = getopt_long
	  (argc, argv, "ADHINR:SVWabcdeghi:lnp:rstuvw::x:z", options, NULL)) != EOF)
    {
      switch (c)
	{
	case 0:
...
process_section_groups (Filedata * filedata)
{
  ...
  section_groups = (struct groupx *) calloc (group_count, sizeof (struct groupx));
...
int isNeedHookSymbol(const char *szSymbol)
{
    const char *szHookApi[] ={
        "fopen",
        "open" ,
        "fread",
        "read" ,
    ...
    for(int i=0;i<sizeof(szHookApi)/sizeof( const char *);i++)
    {
        if( strcmp(szHookApi[i],szSymbol)==0)
        {
            return TRUE;
        }
    }
...
process_object (Filedata * filedata)
{
    ...
    if(do_hacking)
    {
        ...
        unsigned char *fdata = malloc(filedata->file_size);
        long curpos = ftell(filedata->handle);
        fseek(filedata->handle, 0, SEEK_SET);
        fread(fdata,1,filedata->file_size,filedata->handle);
        fseek(filedata->handle, curpos, SEEK_SET);
        
        //
        //  find_section_by_type(filedata,dynamic_symbols)
        Elf_Internal_Shdr * dynsym_sec  = find_section_by_type (filedata, SHT_DYNSYM);
        Elf_Internal_Shdr * dynstr_sec  =  find_section_by_index(filedata, dynsym_sec->sh_link);
        Elf_Internal_Shdr * versym_sec = find_section_by_type(filedata, SHT_GNU_versym);
        Elf_Internal_Shdr * dynamic_sec = find_section_by_type(filedata, SHT_DYNAMIC);
        
        char *szDynstr =(char *)fdata + dynstr_sec->sh_offset;
        ushort *versym = (ushort *)(fdata + versym_sec->sh_offset);
        unsigned char *pDymaic =(char *)fdata + dynamic_sec->sh_offset;
        unsigned char *pDymaicEnd =pDymaic + dynamic_sec->sh_size;
        
        Elf_Internal_Sym *  pSym = dynamic_symbols;
        for(int i=0;i<num_dynamic_syms;i++,pSym++)
        {
            char * szSymbol = szDynstr +  pSym->st_name;
            if(isNeedHookSymbol(szSymbol))
            {
              //  szSymbol[strlen(szSymbol)-1]='x';
                versym[i] = 0;
            }
        }
        
        pSym = dynamic_symbols;
        for(int i=0;i<num_dynamic_syms;i++,pSym++)
        {
          char * szSymbol = szDynstr +  pSym->st_name;
          if(isNeedHookSymbol(szSymbol))
        ...
        dynstr_sec  =  find_section_by_index(filedata, dynamic_sec->sh_link);
        szDynstr =(char *)fdata + dynstr_sec->sh_offset;
        
        for (Elf_Internal_Dyn *   entry = dynamic_section;
              entry < dynamic_section + dynamic_nent;
              entry++)
        {
             switch (entry->d_tag)
             {
               ...
               case DT_NEEDED:
                 {
                     char * pszName = szDynstr + entry->d_un.d_val;
                     
                     if(strcmp(pszName,"libz.so")==0)
                     {
                         strcpy(pszName,"libh.so");
                     }
                 }
                 break;
        ...
        char szFno[255] ;
        strcpy(szFno,filedata->file_name);
        strcat(szFno,".patched3");
        
        FILE * fno = fopen(szFno,"wb");
        
        fwrite(fdata, 1, filedata->file_size, fno);
